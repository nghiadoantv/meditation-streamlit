...
if not st.session_state.logged_in:
    st.title("🧘 Thiền Hơi Thở - Đăng Nhập")
    ...
    st.info("💡 Tạo tài khoản để theo dõi tiến trình thiền của bạn!")
    st.stop()
...
with st.sidebar:
    ...
    st.metric("🔄 Tổng chu kỳ", f"{stats['total_cycles']} chu kỳ")
...
if not st.session_state.audio_generated:
    ...
    # ==================== LƯU SESSION THIỀN ====================
    st.markdown("---")
    st.subheader("💾 Lưu Session Thiền")
    
    st.info("📝 Sau khi thiền xong, điền thông tin và lưu để ghi nhận tiến trình!")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        manual_cycles = st.number_input("Số chu kỳ đã hoàn thành", min_value=1, max_value=999, value=total_cycles if total_cycles > 0 else 10)
    
    with col2:
        estimated_duration = int((manual_cycles * cycle_duration) / 60)
        manual_duration = st.number_input("Thời gian (phút)", min_value=1, max_value=300, value=max(1, estimated_duration))
    
    with col3:
        if two_phase_mode:
            manual_type = st.selectbox("Loại", ["2 Phases", "Phase 1", "Phase 2"])
        else:
            manual_type = st.selectbox("Loại", ["Normal Breathing", "Box Breathing", "Pranayama"])
    
    if st.button("💾 Lưu Session Này", type="primary", use_container_width=True):
        save_session(
            user_id=st.session_state.user_id,
            cycles=manual_cycles,
            duration=manual_duration,
            breathing_type=manual_type
        )
        st.success("✅ Đã lưu session thành công! Streak của bạn đã được cập nhật.")
        st.balloons()
        st.rerun()
...
st.markdown("---")
st.caption("✨ Ứng dụng Thiền Hơi Thở v7.0 - Login + Database + Streak Tracking 🧘")
